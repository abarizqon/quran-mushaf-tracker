 ...
 * quran-page-data.js
 * 
 * Data statis mapping 604 halaman Mushaf Madinah ke Surat, Ayat, dan Juz.
 * Menggunakan array kompak dan fungsi lookup untuk efisiensi dan offline PWA.
 * 
 * Sumber data: Mushaf Al-Madinah An-Nabawiyah (King Fahd Complex / Al-Quran Cloud)
 */

// --- SURAH DATA ---
// Format: [nomor, namaSimple, namaArab, halamanMulai]
const SURAH_DATA = [
...
export function getPageDetails(pageNumber) {
    ...
    return {
        ...
        endSurah: endSurah,
        endAyah: endAyah,
        surah: startSurah, // alias kompatibilitas
        surahName: startSurahObj ? startSurahObj[1] : '',
        surahArabic: startSurahObj ? startSurahObj[2] : '',
        endSurahName: endSurahObj ? endSurahObj[1] : '',
        endSurahArabic: endSurahObj ? endSurahObj[2] : '',
        displayText: displayText,
        isMultiSurah: startSurah !== endSurah,
        juzStartPage: juzStart,
        juzEndPage: juzEnd,
        juzTotalPages: juzEnd - juzStart + 1
    };
}

/**
 * Kompatibilitas dengan getPageInfo terdahulu.
...
export function getPageInfo(pageNumber) {
    ...
    return {
        surah: details.startSurah,
        surahName: details.surahName,
        surahArabic: details.surahArabic,
        juz: details.juz,
        startAyah: details.startAyah,
        endAyah: details.endAyah,
        displayText: details.displayText
    };
}

/**
 * Ambil info lengkap per Juz (halaman mulai, halaman akhir, surat, total halaman).
 * @param {number} juzNumber - Nomor Juz (1-30)
 * @returns {object|null}
 */
export function getJuzInfo(juzNumber) {
    const j = parseInt(juzNumber);
    if (!j || j < 1 || j > 30) return null;

    const startPage = JUZ_DATA[j - 1][1];
    const endPage = j === 30 ? 604 : JUZ_DATA[j][1] - 1;
    const totalPages = endPage - startPage + 1;
    const startDetails = getPageDetails(startPage);
    const endDetails = getPageDetails(endPage);
    ...
    return {
        ...
        startSurahName: startDetails ? startDetails.surahName : '',
        endSurahName: endDetails ? endDetails.endSurahName : '',
        displayText: 'Juz ' + j + ' (Hal ' + startPage + ' - ' + endPage + ')'
    };
}

/**
 * Formula perhitungan ekuivalen Juz dari total halaman.
 * Standar: 20 hal/juz, Juz 1 (21 hal), Juz 30 (23 hal).
 * @param {number} pagesCount
 * @returns {string}
 */
export function calculateJuzFromPages(pagesCount) {
    const count = parseInt(pagesCount);
    if (!count || count <= 0) return '0 Juz';
    if (count >= 604) return '30 Juz (Khatam)';
    ...
    if (remaining >= 21) {
        ...
        return (remaining / 21).toFixed(1) + ' Juz';
    }

    const middleJuz = Math.floor(remaining / 20);
    if (middleJuz < 28) {
        const fraction = (remaining % 20) / 20;
        const total = juz + middleJuz + fraction;
        return total.toFixed(1) + ' Juz';
    }

    juz += 28;
    remaining -= (28 * 20);

    const frac30 = remaining / 23;
    const total = Math.min(30, juz + frac30);
    return total.toFixed(1) + ' Juz';
}

/**
 * Ambil halaman pertama dari surat tertentu.
 * @param {number} surahNumber - Nomor surat (1-114)
 * @returns {number|null}
 */
export function getSurahStartPage(surahNumber) {
    const entry = SURAH_DATA.find(s => s[0] === parseInt(surahNumber));
    return entry ? entry[3] : null;
}

/**
 * Ambil halaman pertama dari juz tertentu.
 * @param {number} juzNumber - Nomor juz (1-30)
 * @returns {number|null}
 */
export function getJuzStartPage(juzNumber) {
    const entry = JUZ_DATA.find(j => j[0] === parseInt(juzNumber));
    return entry ? entry[1] : null;
}

/**
 * Ambil daftar semua surat dengan info halaman.
 * @returns {Array<{id: number, name: string, arabic: string, startPage: number}>}
 */
...
/**
 * Ambil daftar semua juz dengan info halaman.
 * @returns {Array<{id: number, startPage: number, endPage: number, totalPages: number}>}
 */
export function getAllJuz() {
    return JUZ_DATA.map(j => getJuzInfo(j[0]));
}
